...
use App\Models\register_new_request;
...
class RegisterNewRequestController extends Controller
{
    ...
    {
        $data = '';
        return view('backend.tracking.register_request', compact('data'));
    }
    public function create(){

        $data = '' ;
        $tracking_no=Helper::IDGenerator(new register_new_request,'tracking_no',5,date('Ymd'));
        $tracking_type = lTrackingType::all();
        return view('backend.tracking.register_request_create',compact('tracking_no','tracking_type','data'));
    }
